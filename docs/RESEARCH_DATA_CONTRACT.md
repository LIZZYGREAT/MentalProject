...
Rolling-Origin 参数是 evaluation-only，必须保存为 `evaluation_candidate_parameters`、`evaluation_candidate_uncertainty` 和 `evaluation_candidate_evidence`。最终生产参数只能来自 `stage4-deployment-refit.v1`：它只读取同一 immutable Dataset v4 中目标参与者截至 snapshot observation cutoff 的全部 eligible frozen observations/BRS/Slow State，产出独立 `deployment_*` 字段，不得查询 live evidence。LearnedModelProfile 的 window、sample/day count 与 uncertainty 必须对应真实 deployment refit；model-selection provenance 必须包含 dataset snapshot、knowledge cutoff 与 refit version。部署重拟合不得改变 rolling-origin metrics。
