...
Examples:
    ...
    python -m auth.manage create-api-key --username service-a --name production
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    user_state.add_argument("--username", required=True)
    ...
    create_key.add_argument("--username", required=True)
...
def _require_user(database: AppDatabase, username: str) -> dict:
...
def main(argv=None) -> int:
    ...
    try:
        if args.command == "init-db":
            ...
            user = _require_user(database, args.username)
