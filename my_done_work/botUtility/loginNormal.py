# If you find this file, note this is the standalone version, without any access control
# Please use with care, don't do evil.
...
def get_login_session(username,password,service):
    assert service == 'eamis', 'Service Not Supported'
    cur_account = account(username,password)
    session,result = eam_login(cur_account)
    return session

def get_eamis_session(username,password):
    session = get_login_session(username,password,'eamis')
    return session,None
