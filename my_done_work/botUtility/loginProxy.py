...
def login(login_session,url,account):
    if url.startswith(iamurl):
        ...
        t = rsaencrypt(bytes(account.PASSWORD,"utf-8"))
        data = dict(ajax = 1,
                    username = account.ID,
...
def libic_login(user_account,libic_session=None):
    ...
    return libic_session,response

cache = Cache("/home/www/app.LOGIN/.main_cache")
cache.size_limit = 100*1024*1024 #100M
cache.eviction_policy = 'least-recently-used'
cache.expire()

@cache.memoize(expire=30*60)#30min
def get_login_session(username,password,service):
    assert service == 'libic', 'Service Not Supported'
    cur_account = account(username,password)
    session,result = libic_login(cur_account)
    return session

def get_libic_session(username,password):
    session = get_login_session(username,password,'libic')
    response = session.get('https://libic.nankai.edu.cn/ic-web/auth/userInfo')
    return session,response
...
if __name__ == "__main__":
  ...
  sess,res = get_libic_session(user_name,password)
  print(res.text)
  #sess = new_session()
  #s = login(sess,'https://sso.nankai.edu.cn/sso/login?service=https://dzpz.nankai.edu.cn',my_account)
  
  #s = login(sess,'https://iam.nankai.edu.cn/api/tpslogin/link?login_scene=feilian&next=/api/cas/login?service=https://dzpz.nankai.edu.cn&os=web',my_account)
  #print(s)
