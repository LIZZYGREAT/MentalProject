from utils.struct import serialize, unserialize, TypedList, field, REQUIRED, UNREQUIRED, FIXED, NODEFAULT, EMPTY, SERIAL_FAIL, component, dummy
import json
...
class vevent(component):
    '''
    event_id:日程 ID，只读字段，是日程实体的唯一标志。
    organizer_calendar_id:日程组织者的日历 ID，只读字段。
    summary:日程标题
    ...
    status:日程状态，只读字段。
        ...
        tentative：日程暂未回应。
        confirmed：日程已确认。
        cancelled：日程已删除，当该日程是例外日程时，需要额外处理，可参考本文重复日程章节。
    ...
    is_exception:该日程是否是一个重复日程的例外日程，只读字段。
    recurring_event_id:当 is_exception 为 true 时有效，表示该例外日程关联的重复日程的 event_id。
    schemas:日程自定义信息，控制日程详情页的 UI 展示。不传值则默认为空。
    attachements:日程附件。
    event_check_in:日程签到设置，为空则不进行日程签到设置。
    create_time:日程的创建时间（秒级时间戳）
    '''
    _is_concrete = True
    _meta_add = dict(event_id = field(UNREQUIRED,str,EMPTY),
                     summary = field(UNREQUIRED,str,EMPTY),
                     description = field(UNREQUIRED,str,EMPTY),
                     start_time = field(REQUIRED,time_info,True),
                     end_time = field(REQUIRED,time_info,True),
                     status = field(UNREQUIRED,str,EMPTY),
                     is_exception = field(UNREQUIRED,bool,EMPTY),
                     app_link = field(UNREQUIRED,str,EMPTY),
                     location = field(UNREQUIRED,event_location,EMPTY),
                     organizer_calendar_id = field(UNREQUIRED,str,EMPTY),
                     ...
                     recurring_event_id = field(UNREQUIRED,str,EMPTY),
                     ...
                     create_time = field(UNREQUIRED,str,EMPTY),
                     attendees = field(UNREQUIRED,TypedList[str],EMPTY),
                     has_more_attendees = field(UNREQUIRED,bool,False),
                     event_organizer = field(UNREQUIRED,dummy,EMPTY)
                    )

class eventInstance(component):
    _is_concrete = True
    _meta_add = dict(event_id = field(UNREQUIRED,str,EMPTY),
                     summary = field(UNREQUIRED,str,EMPTY),
                     description = field(UNREQUIRED,str,EMPTY),
