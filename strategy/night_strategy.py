...
class NightStrategy(BaseStrategy):
    """
    夜间恢复策略基类
    包含分阶段动力学、稳态弹簧力，以及用一阶自回归产生的连续随机生理信号。
    """
    ...
    def calculate_step(self, S: float, E: float, current_time: datetime, time_step: int, 
                       elapsed_minutes: float = 0.0, step_noise_s: float = 0.0, 
                       step_noise_e: float = 0.0, is_substep: bool = False) -> Tuple[float, float]:
        """
        睡眠段内一步 dS、dE。
        """
...
class NormalNightStrategy(NightStrategy):
    ...
    def calculate_step(self, S: float, E: float, current_time: datetime, time_step: int, 
        ...
        if elapsed_minutes < self.params.get("initial_phase_minutes", 60):
            ...
            delta_S = -decay * diff + 0.3 * self._get_rhythm_factor(elapsed_minutes, 90.0) * (time_step / 5.0) + \
                      self._get_ar1_noise(rho, sigma, step_noise_s, is_substep)
            delta_E = self._calc_saturated_energy(E, S, time_step, multiplier=0.9) + step_noise_e * 0.015
            ...
            delta_S = delta_trend + delta_rhythm + self._get_ar1_noise(rho, sigma, step_noise_s, is_substep)
            delta_E = self._calc_saturated_energy(E, S, time_step, multiplier=1.0) + step_noise_e * 0.015
            return delta_S, delta_E
...
class DeepNightStrategy(NightStrategy):
    ...
    def calculate_step(self, S: float, E: float, current_time: datetime, time_step: int, 
        ...
        sigma = self.cfg.get("sigma", 0.04)
        
        if elapsed_minutes < self.params.get("initial_phase_minutes", 60) - 15:
            delta_S = -self.params.get("initial_decay_rate", 0.02) * 1.3 * diff + \
                      self._get_ar1_noise(rho, sigma, step_noise_s, is_substep)
            delta_E = self._calc_saturated_energy(E, S, time_step, multiplier=1.25) + step_noise_e * 0.01
            return delta_S, delta_E
        else:
            ...
            delta_S = delta_trend + delta_rhythm + self._get_ar1_noise(rho, sigma, step_noise_s, is_substep)
            delta_E = self._calc_saturated_energy(E, S, time_step, multiplier=1.2) + step_noise_e * 0.01
            return delta_S, delta_E
...
class AnxiousNightStrategy(NightStrategy):
    ...
    def calculate_step(self, S: float, E: float, current_time: datetime, time_step: int, 
        ...
        if elapsed_minutes < self.params.get("initial_phase_minutes", 60) + 20: 
            ...
            delta_S = -decay * diff + self._get_ar1_noise(rho, sigma * 1.3, step_noise_s, is_substep) 
            delta_E = self._calc_saturated_energy(E, S, time_step, multiplier=0.85) + step_noise_e * 0.025
            if E + delta_E > 96.0: delta_E = max(0.0, 96.0 - E)
            return delta_S, delta_E
        else:
            if diff <= self.params.get("anchor_threshold", 2.5):
            ...
            delta_S = delta_trend + delta_rhythm + self._get_ar1_noise(rho, sigma, step_noise_s, is_substep)
            delta_E = self._calc_saturated_energy(E, S, time_step, multiplier=0.95) + step_noise_e * 0.025
            if E + delta_E > 96.0: delta_E = max(0.0, 96.0 - E)
