...
class MorningBriefScheduler:
    def __init__(self, *, schedules: Any, participants: Any, bindings: Any, care_preferences: Any, proactive_policy: Any, calendar: Any, sender: Any, reminder_source: Any = None, brief_preferences: Any = None, topic_preferences: Any = None, research_service: Any = None, research_planner: Any = None, research_ranker: Any = None, brief_composer: Any = None, timezone_name: str = "Asia/Shanghai", poll_interval_seconds: int = 60, catch_up_minutes: int = 120, validity_minutes: int = 120, claim_lease_seconds: int = 120, retry_base_seconds: int = 60, max_attempts: int = 5, research_prepare_minutes: int = 15, clock: Callable[[], datetime] | None = None) -> None:
        ...
        self.research_prepare_minutes = max(0, min(int(research_prepare_minutes), 60))
        self.clock = clock or (lambda: datetime.now(timezone.utc))
        self._stop = asyncio.Event()
        self.started = asyncio.Event()
        self._prepared_research: dict[tuple[str, str], tuple[dict[str, list[Any]], bool]] = {}

    async def _research_topics(
        self, participant_id: Any, topics: list[dict[str, Any]], brief_preferences: dict[str, Any]
    ) -> tuple[dict[str, list[Any]], bool]:
        if not topics or self.research_service is None:
            return {}, False
        ...
        async def research_one(job):
            async with semaphore:
                try:
                    ...
                    return job.topic, self.research_ranker.rank(
        ...
        return research_by_topic, research_unavailable
    ...
    async def run_once(self, now: datetime | None = None) -> dict[str, int]:
        ...
        for participant_id in participant_ids:
            ...
            prepare_start = scheduled_local - timedelta(minutes=self.research_prepare_minutes)
            if not prepare_start <= local_now < scheduled_local + timedelta(minutes=self.catch_up_minutes):
                continue
            ...
            counts["ensured"] += 1
            if local_now < scheduled_local and self.research_service is not None:
                cache_key = (str(participant_id), local_now.date().isoformat())
                if cache_key not in self._prepared_research:
                    try:
                        brief_preferences = await asyncio.to_thread(
                            self.brief_preferences.get_preferences, participant_id
                        ) if self.brief_preferences is not None else {
                            "max_research_items": 10, "lookback_hours": 24, "language": "zh-CN",
                        }
                        topics = await asyncio.to_thread(
                            self.topic_preferences.list_topics, participant_id
                        ) if self.topic_preferences is not None else []
                        self._prepared_research[cache_key] = await self._research_topics(
                            participant_id, topics, brief_preferences
                        )
                    except Exception:
                        self._prepared_research[cache_key] = ({}, True)
        ...
        for item in claimed:
            ...
            if topics and self.research_service is not None:
                cache_key = (str(participant_id), item["local_date"])
                cached = self._prepared_research.pop(cache_key, None)
                if cached is None:
                    cached = await self._research_topics(participant_id, topics, brief_preferences)
                research_by_topic, research_unavailable = cached
