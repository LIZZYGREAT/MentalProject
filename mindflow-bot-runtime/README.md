...
工具只生成 participant-bound 待确认计划（`confirmation_stage`），不调用 Provider；真正
...
- `DEEPSEEK_API_KEY`
- `MUTATION_INTENT_API_ENABLED`、`MUTATION_INTENT_API_URL`、`MUTATION_INTENT_API_MODEL`、`MUTATION_INTENT_API_TIMEOUT_SECONDS`、`MUTATION_INTENT_MAX_CONCURRENCY`：用于真正的 Agent 内部/外部写操作授权；Calendar `confirmation_stage` 不调用该语义校验，Provider 写入仍必须经过参与者固定卡确认。不可用或响应非法时其它写操作 fail closed，read/compute/UI/confirmation stage 不受影响。
