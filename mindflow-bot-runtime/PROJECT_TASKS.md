...
| DEP-01 | P0 | 首次构建并启动生产容器 | TODO | `docker compose up --build -d` 成功；`bot`、`admin`、`postgres` 健康；Alembic head `0016_care_intervention_feedback` 与 Agent SDK import 成功 | — | 保存脱敏后的 `docker compose ps`、`alembic current` 和启动日志 |
...
| BASE-07 | DONE | Agent SDK Docker、Claude state volume、Alembic `0016` head 和生产配置模板 | 自动迁移与静态验证（历史完成基线：2026-08-25） |
