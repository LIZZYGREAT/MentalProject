...
RUNNING_ADMIN_REVISION=$(docker compose -f "$COMPOSE_FILE" exec -T admin \
  python3 -c "from app.build_info import BUILD_REVISION; print(BUILD_REVISION)")
TEST_IMAGE_REVISION=$(docker compose -f "$COMPOSE_FILE" run --rm --no-deps bot \
  python3 -c "from app.build_info import BUILD_REVISION; print(BUILD_REVISION)")

if [ "$HOST_REVISION" != "$RUNNING_BOT_REVISION" ] || \
  [ "$HOST_REVISION" != "$RUNNING_ADMIN_REVISION" ] || \
  [ "$HOST_REVISION" != "$TEST_IMAGE_REVISION" ]; then
  echo "revision mismatch: host=$HOST_REVISION running_bot=$RUNNING_BOT_REVISION running_admin=$RUNNING_ADMIN_REVISION test_image=$TEST_IMAGE_REVISION" >&2
  exit 4
fi
